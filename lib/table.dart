...
Future<List<ElementData?>> prepare() {
  WidgetsFlutterBinding.ensureInitialized();
  Future<List<ElementData?>>  gridList = rootBundle.loadString('assets/elementsGrid.json')
...
class ElementsApp extends StatelessWidget {
  ...
  Future<List<ElementData?>> gridList;

  @override
  Widget build(BuildContext context) {
    final theme= ThemeData(brightness: Brightness.dark, primaryColor: Colors.black, elevatedButtonTheme: ElevatedButtonThemeData(style: ElevatedButton.styleFrom(primary: Colors.black, onPrimary: Colors.grey)), textTheme: Typography.whiteMountainView.apply(fontFamily: 'Roboto Condensed'),
      primaryTextTheme: Typography.whiteMountainView.apply(fontFamily: 'Share Tech Mono'),);

    return MaterialApp(title: 'Periodic Table', theme:theme,
...
class TablePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        // leading: IconButton(
        // icon: Icon(Icons.arrow_back, color: Colors.white), onPressed: () => Navigator.pop(context,false),),
        title: Text('Periodic Table'),
        centerTitle: true, elevation: 0,),
      body: FutureBuilder(
        future: gridList,
        builder: ( _,snapshot) => snapshot.hasData ? _buildTable(snapshot.data as List<ElementData?> )
